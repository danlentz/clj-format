...
equivalents. Every example shows the format string, the DSL, the arguments,
and the output. Sources include Practical Common Lisp, CLtL2, ClojureDocs,
and the CL HyperSpec.
...
(clj-format nil ["decimal " :int " binary " :bin " octal " :oct " hex " :hex] 63 63 63 63)
...
*Classic pattern: `~@[` inside `~{}` to skip nil values.*
...
### Extract keys from a plist (skip values with ~*)
...
(clj-format nil [[:each :str [:choose {:selector :# :default ", "} nil ", and "]]] [1 2 3])
;; => "1, 2, and 3"
```
...
*Source: Practical Common Lisp. Back up after `~R` to select the correct suffix
by numeric index.*

```clojure
(cl-format nil "I saw ~R el~:*~[ves~;f~:;ves~]." 0)
(clj-format nil ["I saw " :cardinal " el" :back [:choose {:default "ves"} "ves" "f"] "."] 0)
;; => "I saw zero elves."

(cl-format nil "I saw ~R el~:*~[ves~;f~:;ves~]." 1)
(clj-format nil ["I saw " :cardinal " el" :back [:choose {:default "ves"} "ves" "f"] "."] 1)
;; => "I saw one elf."

(cl-format nil "I saw ~R el~:*~[ves~;f~:;ves~]." 2)
(clj-format nil ["I saw " :cardinal " el" :back [:choose {:default "ves"} "ves" "f"] "."] 2)
;; => "I saw two elves."
```
...
(clj-format nil
  ["Color " :str ", num1 " :int ", num2 " [:int {:width 5 :fill \0}]
...
(cl-format nil "<~A~{~^ ~A=\"~A\"~}~:[~;/~]>~%" "br" [] true)
;; => "<br/>\n"

;; Open tag with attributes
(cl-format nil "<~A~{~^ ~A=\"~A\"~}~:[~;/~]>~%" "div" ["class" "main"] nil)
;; => "<div class=\"main\">\n"
```

### Capitalized key-value report with nil fallback
...
```clojure
(cl-format nil "~:{~@R. ~:(~A~)~%~}" [[1 "first"] [2 "second"] [3 "third"]])

(clj-format nil
  [[:each {:from :sublists} :roman ". " [:str {:case :capitalize}] :nl]]
...
(clj-format nil
  [[:justify {:width 30} "Name" "Count" "Price"] :nl
...
## More Examples from CLtL2 and Practical Common Lisp
...
### Progressive disclosure with :stop
...
(clj-format nil ["I saw "
                 [:choose {:default [:back :cardinal]} "no"]
                 " el" :back
...
*Source: CLtL2. Uses `~#[` (`:choose` with `:selector :#`) to dispatch on
the number of remaining args, selecting none/single/pair/list formatting.
The `:selector :#` means "use the count of remaining arguments as the index."*
...
```clojure
(cl-format nil "Items:~#[ none~; ~S~; ~S and ~S~:;~@{~#[~; and~] ~S~^,~}~].")
;; => "Items: none."

(cl-format nil "Items:~#[ none~; ~S~; ~S and ~S~:;~@{~#[~; and~] ~S~^,~}~]." "foo")
;; => "Items: \"foo\"."

(cl-format nil "Items:~#[ none~; ~S~; ~S and ~S~:;~@{~#[~; and~] ~S~^,~}~]." "foo" "bar")
;; => "Items: \"foo\" and \"bar\"."

(cl-format nil "Items:~#[ none~; ~S~; ~S and ~S~:;~@{~#[~; and~] ~S~^,~}~]." "foo" "bar" "baz" "quux")
...
(cl-format nil english-list [1])       ;; => "1"
(cl-format nil english-list [1 2])     ;; => "1 and 2"
(cl-format nil english-list [1 2 3])   ;; => "1, 2, and 3"
(cl-format nil english-list [1 2 3 4]) ;; => "1, 2, 3, and 4"
```
...
### Justification with padding before first segment
